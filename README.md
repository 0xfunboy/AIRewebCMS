...
- Mirror/Optimize actions report accurate totals even on legacy browsers (fallback messaging keeps admins informed when counts are unavailable).
- Admin JavaScript (dashboard + inline editor) now lives in `/assets/js/admin.js` and avoids bleeding-edge syntax, so legacy shared-hosting browsers keep running without parse errors.
