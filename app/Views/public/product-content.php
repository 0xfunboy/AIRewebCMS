...
<div>
    <div class="text-center mb-12" data-animate>
        ...
        <h1 class="text-4xl md:text-5xl font-extrabold text-acc tracking-tight"><?= htmlspecialchars($product['hero_title'] ?: $product['name'], ENT_QUOTES, 'UTF-8'); ?></h1>
        <?php if (!empty($product['hero_subtitle'])): ?>
            <p class="mt-4 max-w-3xl mx-auto text-lg text-muted"><?= htmlspecialchars($product['hero_subtitle'], ENT_QUOTES, 'UTF-8'); ?></p>
        <?php endif; ?>
    </div>
    ...
    <?php if ($ctaLink && $ctaText): ?>
        <div class="mt-12 text-center" data-animate>
            <a href="<?= htmlspecialchars($ctaLink, ENT_QUOTES, 'UTF-8'); ?>" target="_blank" rel="noopener noreferrer" class="bg-pri text-white font-bold py-3 px-8 rounded-md hover:bg-pri-700 transition-colors text-lg">
