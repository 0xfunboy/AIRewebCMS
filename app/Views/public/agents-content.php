                ...
                <div class="bg-glass border border-stroke rounded-lg overflow-hidden transition-all duration-300 transform hover:-translate-y-1 hover:border-pri/50 shadow-deep backdrop-blur-lg h-full">
                    ...
                    <div class="p-6 flex flex-col">
                        <div class="flex justify-between items-start mb-2">
                            ...
                            <span class="text-xs font-semibold px-2 py-1 rounded-full <?= $statusClass; ?>"><?= htmlspecialchars($status, ENT_QUOTES, 'UTF-8'); ?></span>
                        </div>
                        <div class="text-sm font-semibold text-cy mb-3"><?= htmlspecialchars($agent['chain'], ENT_QUOTES, 'UTF-8'); ?></div>
                        <p class="text-muted text-sm mb-4 flex-grow leading-relaxed"><?= htmlspecialchars($agent['summary'], ENT_QUOTES, 'UTF-8'); ?></p>
                        <a href="<?= htmlspecialchars($agent['site_url'], ENT_QUOTES, 'UTF-8'); ?>" target="_blank" rel="noopener noreferrer" class="bg-pri/20 text-pri font-bold py-2 px-4 rounded-md text-sm hover:bg-pri/40 transition-colors w-full block text-center mt-auto">
                            Visit Agent
                        </a>
                    </div>
                </div>
